...
IRepository<SGOOnMapDbInfo>* SGOOnMapRepository::Initialize(std::string connectionName)
{
  ...
  createTableString = QString("%1 %2 REAL NOT NULL, %3 INTEGER NOT NULL, %4 INTEGER NOT NULL, FOREIGN KEY(%5) REFERENCES %6(%7) ON DELETE CASCADE ON UPDATE CASCADE)").arg(createTableString, m_tableMetadata->GetColumnNames()[7], m_tableMetadata->GetColumnNames()[8], m_tableMetadata->GetColumnNames()[9], m_tableMetadata->GetColumnNames()[0], m_tableMetadata->GetRelationShips().begin()->first, m_tableMetadata->GetRelationShips().begin()->second.OtherTablePrimaryKeyName);
