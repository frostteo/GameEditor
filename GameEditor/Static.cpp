...
void Static::Render(ID3D11DeviceContext* deviceContext, XMMATRIX& viewMatrix, XMMATRIX& projectionMatrix)
{
  m_mesh->Render(deviceContext);

  XMMATRIX worldMatrix;
  this->GetWorldMatrix(worldMatrix);
  m_shader->Render(deviceContext, m_mesh->GetIndexCount(), worldMatrix, viewMatrix, projectionMatrix, m_material);
}