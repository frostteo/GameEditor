...
void PointLightTesselatedShader::InitializeShader(ID3D11Device* device, HWND hwnd, const std::wstring& vsFilename, const std::wstring& hsFilename, const std::wstring& dsFileName, const std::wstring& psFilename)
{
  HRESULT result;
...
void PointLightTesselatedShader::Render(ID3D11DeviceContext* deviceContext, int indexCount, XMMATRIX worldMatrix, XMMATRIX viewMatrix, XMMATRIX projectionMatrix, IMaterial* material, LightininigSystem* lightining, XMFLOAT3& cameraPosition)
{
  ...
  pointLightProjection = XMMatrixMultiply(pointLightWorldMatrix, XMMatrixTranslation(-cameraPosition.x, -cameraPosition.y -cameraPosition.z, 1.0f));
  pointLightProjection = XMMatrixMultiply(pointLightWorldMatrix, projectionMatrix);

  SetShaderParameters(deviceContext, pointLightProjection, pointLight->GetColor(), positionFloat3, pointLight->GetLinearAttenuation(), pointLight->GetQuadraticAttenuation(), pointLightDefferedParameters->GetPerspectiveValues(), pointLightDefferedParameters->GetViewMatrixInverse());
